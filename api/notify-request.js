...
export default async function handler(req, res) {
  ...
  if (!RESEND_API_KEY || !ADMIN_EMAIL) {
    console.error("[Resend API] Missing RESEND_API_KEY or ADMIN_EMAIL env vars.");
    return res.status(500).json({ error: "Server configuration is missing." });
  }

  const resend = new Resend(RESEND_API_KEY);
  // TODO: Use verified sender domain instead of onboarding@resend.dev once domain is verified
