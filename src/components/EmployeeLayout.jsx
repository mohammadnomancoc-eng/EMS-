...
// ── Leave/WFH quota bar shown in sidebar ─────────────
  ...
  return (
    <div className="mb-3">
      <div className="flex justify-between items-center mb-1">
        <span style={{ fontFamily: "Mulish, sans-serif", fontSize: "11px", color: "#A0A0A0" }}>{label}</span>
...
function EmployeeSidebar() {
  ...
  const user = userRaw ? JSON.parse(userRaw) : { name: "Employee", role: "Staff", initials: "EM" };

  // Mock monthly quota — in real app this comes from API
  const quota = {
      ...
      <nav className="flex-1 overflow-y-auto py-3" style={{ scrollbarWidth: "none" }}>
        ...
        <div className="mx-3 mt-4">
          ...
          >
            ...
            <QuotaBar label="Leave"  taken={quota.leave.taken} total={quota.leave.total} color="#CC0000" />
            <QuotaBar label="WFH"    taken={quota.wfh.taken}   total={quota.wfh.total}   color="#00B8B8" />
          </div>
        </div>
      </nav>
...
function EmployeeHeader() {
  ...
  const page = pageTitles[pathname] || { title: "Portal", crumb: "EMPLOYEE / PORTAL" };
  const userRaw = localStorage.getItem("rwt-user");
  const user = userRaw ? JSON.parse(userRaw) : { initials: "EM" };
